...
$tpl = new Template ( 'faturamento-ambiente-processa.html' );
...
if (! empty ( $coligada ) && sizeof ( $movimentos ) > 0 && ! empty ( $ambiente )) {
    ...
    TTMV
SET
    EDICAONUMEROMOV = '$tipo',
    BUSCARNUMEROMOV = 'G'
WHERE
    CODCOLIGADA IN ($coligada) AND
    CODTMV IN ($itens);

UPDATE TTMVEXT SET NFEESTADUAL = $ativo WHERE CODCOLIGADA IN ($coligada) AND CODTMV IN ($itens);
";
	...
	if ($affected > 0) {
		...
		$assunto .= strtoupper ( $_SESSION ['usuario'] ) . " colocou movimento(s) em ";
		...
		//$conteudo .= "<p>Computador: " . strtoupper ( gethostbyaddr ( $_SERVER ['REMOTE_ADDR'] ) ) . "</p>";
		...
		$mail->AddAddress ( 'moliveira@diariodopara.com.br' );
		$mail->AddAddress ( 'robson.correa@diariodopara.com.br' );
