...
";
if (! empty ( $reduzido )) {
	$sql .= "CPARTIDA.{$situacao} = (SELECT c.CODCONTA FROM CCONTA c (nolock) WHERE c.REDUZIDO = $reduzido AND c.CODCOLIGADA = $coligada),";
}
...
if ($affected > 0) {
	...
	if (($data1 != $data_original) or (! empty ( $reduzido )) or ($valor_original != $valor) or ($complemento != $complemento_original)) {
		...
		if ($arquivo) {
			if ($data1 != $data_original) {
				$texto_data = "data de contabilização de $data_original para $data1";
			} else {
				$texto_data = "";
			}
	...
	$tpl->MSG = "
					<h1><font color = 'red'>NÃO FOI POSSÍVEL ALTERAR CONTA. TENTE NOVAMENTE, POR FAVOR!</fonte></h1>
					<a href = 'lista-partidas.php?coligada=$coligada&lancamento=$partida&opcao=$opcao&lote=$lote'>Voltar</a>
	";
}
